...
class TugasAkhirController extends BaseController {
    ...
    function dasborTugasAkhir()
    {
        ...
        if(Request::isMethod('get'))
        {
            ...
            if($auth->peran == 0)
            {
                return Response::json(TugasAkhir::with('penawaranJudul', 'mahasiswa')->where('tugas_akhir.nrp_mahasiswa', $auth->nomor_induk)->get());
            }
            else
            {
                return Response::json(TugasAkhir::with('penawaranJudul', 'mahasiswa')->get());
            }
        }
