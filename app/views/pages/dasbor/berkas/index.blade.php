...
app.controller('daftarBerkasController', function($scope, $http, $rootScope) {
    $scope.hapus = function(id_berkas_tugas_akhir) {
        if(confirm('Yakin hapus berkas tersebut')) {
            $http.delete('{{URL::to('/dasbor/mahasiswa/berkas')}}', {params: {'id_berkas_tugas_akhir': String(id_berkas_tugas_akhir)}}).success(function() {
                alert('Penghapusan berkas berhasil')
                update($rootScope, $http);
            });
        }
    };
...
<div class="row">
    <div class="col-md-12">
        <h1 class="page-header">Kelola dan Unggah Berkas</h1>
    </div>
</div>
<div ng-app="dasborBerkas">
    ...
    <script type="text/ng-template" id="unggahBerkas.html">
        <div ng-controller="unggahBerkasController">
            <div class="col-md-12">
                ...
                <button ng-click="unggahBerkas()" class="btn">Unggah Berkas</button>
            </div>
        </div>
    </script>
</div>
